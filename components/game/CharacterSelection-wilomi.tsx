...
const CharacterSelection: React.FC<CharacterSelectionProps> = ({
  ...
  if (gameData && gameData.currentDayStep === 4 && getNextCharacterToReveal && typeof isCurrentUserGameMaster === 'boolean') {
    ...
    return (
        ...
        <CardContent className="space-y-4">
            {revealedCharacterInfo && (
              <div className="relative w-full h-[32rem] bg-muted/30 rounded-md overflow-hidden border-2 border-border shadow-md mb-4">
                <Image
                  src={revealedCharacterInfo.imageUrl}
                  alt={revealedCharacterInfo.name}
                  fill
                  style={{ objectFit: 'cover', objectPosition: 'top' }}
                  data-ai-hint={revealedCharacterInfo.dataAiHint}
                  sizes="(max-width: 640px) 90vw, (max-width: 1024px) 45vw, 30vw"
                />
              </div>
            )}
            {revealedCharacterInfo && playersWhoChoseRevealedCharacter.length > 0 && (
            <>
                <p className="font-body font-semibold mb-2">Players who chose {revealedCharacterInfo?.name}:</p>
                <ul className="list-disc pl-5 space-y-1 font-body">
                {playersWhoChoseRevealedCharacter.map(player => (
                    <li key={player.id} className="text-muted-foreground">{player.screenName}</li>
                ))}
                </ul>
            </>
            )}
            <PlayerSelectionStatusList
              ...
              isEmbedded // Indicate it's embedded for potentially different styling/less margin
            />
        </CardContent>
        </Card>
    );
                ...
                {availableTargets.map((character) => (
                  <div key={character.id} className="flex-[0_0_100%] min-w-0">
                    <CharacterDisplayCard
                      ...
                      showNavButtons={true}
                      onPrevClick={thiefScrollPrev}
                      onNextClick={thiefScrollNext}
                      isPrevDisabled={thiefSelectedIndex === 0}
                      isNextDisabled={thiefSelectedIndex === availableTargets.length - 1}
                    />
                  </div>
                ))}
              </div>
  ...
  return (
    <Card className="shadow-parchment bg-card hunch-box">
      <CardHeader>
        <CardTitle className="text-xl font-headline text-primary flex items-center">
          <ShieldQuestion className="w-6 h-6 mr-2" />
          Day {currentDay}, Step {displayRoundNumber}: Choose Character
        </CardTitle>
        <CardDescription className="font-body">
          {forcedCharacterId
            ? "Your character has been chosen for you today. Confirm your role."
            : "Swipe left or right to browse characters. Select one whose abilities you wish to use for today."
          }
        </CardDescription>
      </CardHeader>
      <CardContent>
        {forcedCharacterId && (
            <div className="p-3 mb-4 rounded-md border border-amber-600/50 bg-amber-100/60 text-amber-900/90 text-sm flex items-center gap-3">
                <AlertTriangle className="h-6 w-6 shrink-0"/>
                <p>A Magician has enchanted you! You must select <strong className="font-bold">{CHARACTERS_LIST.find(c => c.id === forcedCharacterId)?.name}</strong> for today's round.</p>
            </div>
        )}
              ...
              {characters.map((character, index) => (
                <div key={character.id} className="flex-[0_0_100%] min-w-0">
                  <CharacterDisplayCard
                    ...
                    isNextDisabled={selectedIndex === characters.length - 1}
                  />
                </div>
              ))}
            </div>
